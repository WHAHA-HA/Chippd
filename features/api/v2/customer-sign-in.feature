Feature: Customer Signin
  ...
  Scenario: The client POSTs invalid information
    ...
    When the client requests POST "/2/customer/sign_in" with a request body of:
      """
      {
        "some" : "invalid",
        "json" : "data"
  ...
  Scenario: The client POSTs valid information
    Given the client has API access
    And a customer with the first_name: "PJ", last_name: "Kelly", email: "test@test.com", and password: "s3cr3t" exists
    When the client requests POST "/2/customer/sign_in" with a request body of:
      """
      {
        "email" : "test@test.com",
