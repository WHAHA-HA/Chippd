...
describe GenericPdfUploadHandler do

  let(:customer) { double('customer', :id => 1) }
  let(:section) { double('section', :id => 1, :activate! => true, :file_url= => true) }
  let(:upload_params) {
  ...
  describe "#handle" do
    ...
    it "sets the original_url from params" do
      ...
      section.should_receive(:file_url=).with((JSON.parse upload_params[:upload]).first['url'])
      subject
    end
